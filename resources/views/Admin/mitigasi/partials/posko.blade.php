<div class="container-fluid">
    ...
    <div class="rounded-xl border border-gray-200 bg-white p-6">
        <form action="{{ route('posko.store') }}" method="POST" class="space-y-6">
            ...
            {{-- Jenis Posko --}}
            <x-form.form-elements.default-inputs label="Jenis Posko" name="jenis_posko"
                placeholder="Contoh: Kesehatan, Logistik" value="{{ old('jenis_posko') }}" required />
            ...
            <div class="mb-3">
                ...
                <select id="district_id" name="district_id" class="w-full" style="width: 100%"></select>
            </div>

            <div class="mb-3">
                <label class="block text-sm font-medium mb-1">Desa</label>
                <select id="village_id" name="village_id" class="w-full" style="width: 100%"></select>
            </div>

            {{-- Koordinat --}}
            <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                <x-form.form-elements.default-inputs label="Latitude" name="latitude" placeholder="-6.200000"
                    value="{{ old('latitude') }}" />

                <x-form.form-elements.default-inputs label="Longitude" name="longitude" placeholder="106.816666"
                    value="{{ old('longitude') }}" />
            </div>
